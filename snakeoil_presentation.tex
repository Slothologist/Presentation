...
\begin{document}
	...
	\begin{frame}{Nullinteraktion - Diagramm}
		...
		\end{figure}
		 ...
		 \begin{itemize}
			\item oben links: Unterschiedliche Ausprägung der Faktorstufe haben gleiche Werte\\ 
			\item oben rechts UV ``Präsentationsart'' unterscheidet sich von UV ``Wortart'' aber keine Abhängigkeit voneinander
			\item unten links Umgekehrter Fall zu oben rechts
			\item unten rechts oben rechts und unten links kombiniert. Trotzdem parallel $\rightarrow$ keine Interaktion. Betrachtung der Daten legt ebenfalls nahe, dass keine Abhängigkeit besteht
		\end{itemize}
		 }
	\end{frame}
	...
	\begin{frame}{ordinale Interaktion}
		...
		\end{itemize}
		 \note{Umgangssprachlich: gegenseitig verstärkende Interaktion}
	\end{frame}
	...
	\begin{frame}{semidisordinale Interaktion}
		\note{Frage ans Publikum: Wenn sich bei ordinaler I die Linien nie kreuzen, bei disordinaler I aber immer, was könnte dann die semidis I sein?} 
		\begin{itemize}
	...
	\begin{frame}{Alternativdarstellung}
		\note{Unterschied zwischen der ursprünglichen und alternativen darstellung:\\
		alt: kreutzen der linien essentiell für disordinale Interaktion\\
		alternativ: richtung der linien essentiell, überschneidung kann vorkommen, muss aber nicht}
		Experiment
		\begin{itemize}
	...
	\begin{frame}{Skalenniveau}
		\begin{itemize}
			...
			\item \alert{allerdings:} Manchmal lässt sich aus einer sinnvollen Interpretation auch auf eine sinnvolle Anwendung ``Rückschließen'' \note{3. hier heiligt der swag die Mittel roflcopter\\}\\
			$\rightarrow$ bspw. Schulnoten\note{4. Schulnoten: Ist 1 doppelt so gut wie 2? 4x so gut wie 4? Noten in Prozent angeben $\rightarrow$ beim Programmieren: Erste 80\% der Anforderungen brauchen 20\% der Zeit, die nächsten 20\% 80\% der Zeit}
			\note{anmerken, dass für die VA die ursprüngliche Population natürlich auch normalverteilt sein muss, bei anderen Verteilungen macht die VA keinen Sinn}
		\end{itemize}
	\end{frame}
	
	\section{Live Versuch}
